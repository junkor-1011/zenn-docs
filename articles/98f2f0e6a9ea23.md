...
更に、TauriにおけるUI側とアプリ側の情報のやり取りは、内部的にserdeを使って行っているため
`cargo add serde -F derive`によって、`serde`も入れておく。
...
この記事を書き始めてから存在に気づいたので😔残念ながら詳細は良くわからないものの、便利そう
...
が、執筆時点での最新版が[v0.2.0](https://github.com/specta-rs/rspc/releases/tag/v0.2.0)(2024-03-18)とやや古くなっており、Tauri v2でうまく動いてくれるのかは未検証。
