...
(バックグラウンドで実行されるので、実行状況は`/var/log/cloud-init.log`などを見て確認する)
...
WSL2は偉大だが、稀にWSL特有の制約事項で困るケースがゼロではない(ただ、稀)なので、
どうしても本物のLinuxを使いたかったりするときや、
あるいはセキュリティ的にホストとの隔離度を強くしたい場合には使えそう

意外とHyper-V特有の話は少ないので、例えばLinuxのデスクトップを母艦にして、
kvmで作業環境を作る、といった場合にもほとんど同様の手順でできる。
